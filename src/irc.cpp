...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SficoinTEST2\r");
            Send(hSocket, "WHO #SficoinTEST2\r");
        } else {
            // randomly join #Sficoin00-#Sficoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Sficoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Sficoin%02d\r", channel_number).c_str());
        }
